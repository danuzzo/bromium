...
impl <'a> XMLAttributes<'a> {
    ...
    pub fn into_iter(self) -> impl Iterator<Item = Result<(&'a str, &'a str), Error>> {
    ...
    pub fn remove(&mut self, name: &str) -> Option<(&'a str, &'a str)>  {
    ...
    pub fn get_all(&self) -> Vec<(&'a str, &'a str)> {
        self.attributes.iter().map(|attr| (attr.name, attr.value)).collect()
    }
...
#[derive(Debug, Clone)]
pub struct XMLDomNode<'a> {
    pub name: String,
    pub attributes: XMLAttributes<'a>,
    pub text: Option<String>,
    pub children: Vec<XMLDomNode<'a>>,
}
...
impl <'a> XMLDomNode<'a> {
    ...
    pub fn add_child(&mut self, child: XMLDomNode<'a>) -> &mut Self {
        self.children.push(child);
        self.children.last_mut().unwrap()
    }

    pub fn set_attribute(&mut self, name: &'a str, value: &'a str) {
        self.attributes.set(name, value);
    }
}

pub struct XMLDomWriter<'a> {
    root: Option<XMLDomNode<'a>>,
}

impl <'a> XMLDomWriter<'a> {
    pub fn new() -> Self {
        XMLDomWriter { root: None }
    }

    pub fn set_root(&mut self, node: XMLDomNode<'a>) {
        self.root = Some(node);
    }

    pub fn get_root_mut(&mut self) -> Option<&mut XMLDomNode<'a>> {
        self.root.as_mut()
    }
